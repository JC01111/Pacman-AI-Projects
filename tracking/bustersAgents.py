...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        mostLikelyPositions = []
        # Get the mostLikelyPositions from each list
        for dist in livingGhostPositionDistributions:
            mostLikelyPos = max(dist, key=dist.get)
            mostLikelyPositions.append(mostLikelyPos)

        distance = float('inf')
        final_action = None
        for Pos in mostLikelyPositions: # Find the closest Position
            for action in legal:
                successorPosition = Actions.getSuccessor(pacmanPosition, action) # Use the Action of Pacman
                new_distance = self.distancer.getDistance(Pos, successorPosition)
