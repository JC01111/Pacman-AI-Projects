...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            temp_lst.append((i, j))
    # Add all the possible Pos to PAC, GHOST0, GHOST1
    variableDomainsDict[PAC] = temp_lst
    variableDomainsDict[GHOST0] = temp_lst
    variableDomainsDict[GHOST1] = temp_lst
    # Set to avoid duplicate
    OBS0_lst = set()
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST0]:
            for k in range(MAX_NOISE + 1):
                if (manhattanDistance(i, j) + k) >= 0:
                    OBS0_lst.add(manhattanDistance(i, j) + k)
                if (manhattanDistance(i, j) - k) >= 0:
                    OBS0_lst.add(manhattanDistance(i, j) - k)
    variableDomainsDict[OBS0] = OBS0_lst
    # Add OBS1
    OBS1_lst = set()
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST1]:
            for k in range(MAX_NOISE + 1):
                if (manhattanDistance(i, j) + k) >= 0:
                    OBS1_lst.add(manhattanDistance(i, j) + k)
                if (manhattanDistance(i, j) - k) >= 0:
                    OBS1_lst.add(manhattanDistance(i, j) - k)
    variableDomainsDict[OBS1] = OBS1_lst
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Eliminate each variable in the eliminationOrder
        for var in eliminationOrder:
            if var not in queryVariables:
                notJoinedFactor, joinedFactor = joinFactorsByVariable(factors, var)
                if joinedFactor:
                    if len(joinedFactor.unconditionedVariables()) > 1 or var in joinedFactor.conditionedVariables():
                        # Check if var in uncond, then we eliminate it
                        eliminatedFactor = eliminate(joinedFactor, var)
                        factors = notJoinedFactor + [eliminatedFactor]
                    else:
                        # If notJoinedFactor only has one, we discard factors
                        factors = notJoinedFactor
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        sum = self.total() # Get distribution
        if sum == 0:
            return
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        random_val = random.random() * self.total() # Get a random value from sample * total_val
        curr_sum = 0
        for key, value in self.items():
            curr_sum += value
            if curr_sum > random_val:
                return key
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            obs_Prob = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            self.beliefs[ghostPosition] = prior * obs_Prob
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        Positions = self.allPositions # Get all the previous positions
        newBeliefs = DiscreteDistribution() # Create a new constructor for newBeliefs to update

        for pos in Positions:
            prior = self.beliefs[pos]
            newPosDist = self.getPositionDistribution(gameState, pos)
            for newPos, newProb in newPosDist.items(): # Iterate through all the newPosDict tuples
                if newPos in newBeliefs:
                    newBeliefs[newPos] += prior * newProb
                else:
                    newBeliefs[newPos] = prior * newProb

        self.beliefs = newBeliefs # Update the old beliefs
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(num):
            index = i % len(Positions) # Use the remainder as index
            self.particles.append(Positions[index])
    ...
    def getBeliefDistribution(self):
        ...
        for Pos in self.legalPositions:
            newBeliefs[Pos] = self.particles.count(Pos) / num
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            if particle is not None:
                ...
                if particle in newBeliefs:
                    newBeliefs[particle] += prob
                else:
                    newBeliefs[particle] = prob
        # Check for special case when all particles receive zero weight
        if newBeliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(num):
                new_list.append(newBeliefs.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            if particle is not None:
                newPosDict = self.getPositionDistribution(gameState, particle)
                new_list.append(newPosDict.sample()) # Directly sample from newPosDict
            else:
                new_list.append(jailPosition)  # Is None when in jail
