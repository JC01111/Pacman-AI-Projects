...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        # f(x) = ReLU(xw1+b1)*w2 + b2
        xw1 = nn.Linear(x, self.w1)
        xw1_b1 = nn.AddBias(xw1, self.b1)
        inter = nn.ReLU(xw1_b1)
        inter_w2 = nn.Linear(inter, self.w2)
        interw2_b2 = nn.AddBias(inter_w2, self.b2)
        return interw2_b2
    ...
    def train(self, dataset):
        ...
        while True:
            ...
            if nn.as_scalar(loss) <= 0.02: # Break when loss is 0.02 or better
                break
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        for epoch in range(epochs): # Otherwise it will never stop
            for x, y in dataset.iterate_once(batch_size):
                loss = self.get_loss(x, y)
                gradients = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2])
                self.w1.update(gradients[0], -learning_rate)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        "*** YOUR CODE HERE ***"
        # 3 layers
        h = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w1), self.b1)) # Initialize the first word
        for x in xs[1:]:
            h = (nn.ReLU(
                nn.AddBias(
                    nn.Add(
                        nn.Linear(x, self.w1),
                        nn.Linear(h, self.w2)), self.b2)))

        h = nn.AddBias(nn.Linear(h, self.w3), self.b3)
        return h
